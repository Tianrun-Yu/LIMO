...
"""
Q1 – Top-K Entropy scorer.

Computes the mean top-K entropy of the student model's predictions over
assistant token positions for every (candidate, problem) pair and writes
the full C×N score matrix to scores.json.

Energy function:
  H_K(k) = -Σ_{t ∈ top-K} p̃(t|c_k) log p̃(t|c_k)
  score(τ) = (1/T_asst) Σ_k H_K(k)    (higher = more uncertain = more informative)

Output:
  out_root/<student>/topk_entropy/scores.json
...
Usage (single GPU):
  python score_topk_entropy_baseline.py \\
...
# ---------------------------------------------------------------------------
# Top-K entropy scoring: fully vectorised batched forward pass
# ---------------------------------------------------------------------------

@torch.no_grad()
def score_batch_topk_entropy(
    ...
    # Restrict to top-K logits and compute renormalised entropy
    asst_logits = logits[:, :-1, :]                            # [B, T-1, V] view
    topk_vals, _ = torch.topk(asst_logits, top_k, dim=-1)     # [B, T-1, K]
    del logits, asst_logits

    log_p   = F.log_softmax(topk_vals, dim=-1)                # [B, T-1, K]
    p       = log_p.exp()
    entropy = -(p * log_p).sum(dim=-1)                        # [B, T-1]
    ...
    for b in range(B):
        ...
        if lp_start < 0 or lp_start >= lp_end:
            scores.append(0.0)
            continue
        scores.append(float(entropy[b, lp_start:lp_end].mean().item()))
        ...
        for fidx, (teacher, fp) in enumerate(cand_files):
            ...
            def flush_batch():
                if not batch_items:
                    return
                def _score(items):
                    return score_batch_topk_entropy(model, tokenizer, items, top_k, max_length, device)
                try:
...
def main():
    ap = argparse.ArgumentParser(
        description="Q1 Top-K Entropy baseline: mean top-K entropy of student predictions."
    )
    ...
    for s in students:
        out_dir  = os.path.join(args.out_root, s, "topk_entropy")
