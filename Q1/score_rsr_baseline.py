...
"""
Q1 – Rank-Surprisal Ratio (RSR) scorer.

Computes RSR for every (candidate, problem) pair and writes the full C×N
score matrix to scores.json. No selection is performed here (that is Q2).

Energy function (paper Appendix C.1):
  s_k = logsumexp(logits_k) - logit_k[t_k]          (surprisal, ≥ 0)
  r̄_k = min(rank(t_k), r_max)                        (clipped 1-indexed rank)
  RSR(τ) = Σ_k r̄_k / Σ_k s_k                        (lower = more natural)

Output:
  out_root/<student>/rsr/scores.json
...
# ---------------------------------------------------------------------------
# RSR scoring: fully vectorised batched forward pass
# ---------------------------------------------------------------------------
...
def score_batch_rsr(
    ...
    # Vectorised surprisal: logsumexp - logit[target]  (avoids full log_probs tensor)
    asst_logits   = logits[:, :-1, :]                                    # [B, T-1, V] view
    targets       = input_ids[:, 1:]                                     # [B, T-1]
    target_logits = asst_logits.gather(2, targets.unsqueeze(2)).squeeze(2)  # [B, T-1]
    logsumexp     = torch.logsumexp(asst_logits, dim=-1)                 # [B, T-1]
    surprisal     = (logsumexp - target_logits).float()                  # [B, T-1]

    # Vectorised clipped rank via topk + boolean match
    _, topk_ids = torch.topk(asst_logits, r_max, dim=-1)                # [B, T-1, r_max]
    del logits, asst_logits

    match     = (topk_ids == targets.unsqueeze(2))                       # [B, T-1, r_max]
    any_match = match.any(dim=2)
    rank_0idx = match.long().argmax(dim=2)
    rank_clip = torch.where(
        any_match,
        rank_0idx + 1,
        torch.full_like(rank_0idx, r_max),
    ).float()                                                            # [B, T-1]
    ...
    for b in range(B):
        ...
        if lp_start < 0 or lp_start >= lp_end:
            scores.append(0.0)
            continue
        sum_surp = surprisal[b, lp_start:lp_end].sum().item()
        sum_rank = rank_clip[b, lp_start:lp_end].sum().item()
        if sum_surp <= 0.0:
            scores.append(0.0)
        else:
            scores.append(sum_rank / sum_surp)
        ...
        for fidx, (teacher, fp) in enumerate(cand_files):
            ...
            def flush_batch():
                if not batch_items:
                    return
                def _score(items):
                    return score_batch_rsr(model, tokenizer, items, r_max, max_length, device)
                try:
...
def main():
    ap = argparse.ArgumentParser(
        description="Q1 RSR baseline: Rank-Surprisal Ratio per (candidate, problem)."
    )
    ...
    ap.add_argument("--r_max",         type=int, default=100)
    ...
    for s in students:
        out_dir  = os.path.join(args.out_root, s, "rsr")
